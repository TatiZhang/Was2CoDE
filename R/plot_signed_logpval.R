...
plot_signed_logpval <- function(df, 
                                method1, 
                                method2,
                                bool_capped = TRUE,
  ...
  stopifnot(all(c("Feature", 
                  ...
                  paste0(method2, "_pval")) %in% colnames(df)))
  ...
  names(x_val) <- names(y_val) <- df$Feature
  
  # FDR cutoffs
  if(bool_multtestadjust){
    x_pval <- stats::p.adjust(df[, paste0(method1, "_pval")], method = "BH")
  ...
  pc1_slope <- stats::prcomp(cbind(x_val, y_val), center = FALSE)$rotation[2,1] /
    stats::prcomp(cbind(x_val, y_val), center = FALSE)$rotation[1,1]
  
  # Highlighting DE genes/pathways when highlight_features == NULL
  if (all(is.null(highlight_features))) {
    # If no specific pathways provided, highlight genes significant in both methods
    sig_both <- unique(c(x_features, y_features))
    features_to_highlight <- sig_both
  } else {
    # Use provided pathways for highlighting
    features_to_highlight <- highlight_features
  }
  ...
  ggplot_df <- data.frame(
    ...
    method1 = x_val,
    method2 = y_val,
    highlight = ifelse(names(x_val) %in% features_to_highlight, "highlight", "other"),
    label = ifelse(names(x_val) %in% features_to_highlight, names(x_val), "")
  )
  
  # Fisher enrichment test
  if (!is.null(highlight_features)) {
    highlight_set <- intersect(highlight_features, ggplot_df$feature)
    sig_features <- intersect(x_features, y_features)
    a <- sum(highlight_set %in% sig_features)
    b <- sum(highlight_set %in% ggplot_df$feature) - a
    c <- sum(sig_features %in% ggplot_df$feature) - a
    d <- nrow(ggplot_df) - a - b - c
    fisher_matrix <- matrix(c(a, b, c, d), nrow = 2)
    fisher_pval <- stats::fisher.test(fisher_matrix)$p.value
  } else {
    fisher_pval <- NA
  }
  ...
  p <- ggplot2::ggplot(ggplot_df, ggplot2::aes(x = method1, y = method2)) +
    ...
    ggplot2::geom_abline(slope = pc1_slope, 
                         ...
                         color = "coral", 
                         linewidth = 1) +
    ggplot2::geom_hline(yintercept = c(-y_fdrcutoff, y_fdrcutoff), 
                        linetype = "dashed") +
    ggplot2::geom_vline(xintercept = c(-x_fdrcutoff, x_fdrcutoff), 
                        linetype = "dashed") +
